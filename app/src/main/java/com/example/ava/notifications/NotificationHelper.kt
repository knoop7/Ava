...
private const val SCREENSAVER_SERVICE_CHANNEL_ID = "ScreensaverService"
...
fun createScreensaverServiceNotificationChannel(context: Context) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        val channelName = "Screensaver Service"
        val chan = NotificationChannel(
            SCREENSAVER_SERVICE_CHANNEL_ID,
            channelName,
            NotificationManager.IMPORTANCE_LOW
        )
        chan.lightColor = Color.BLUE
...
fun createScreensaverServiceNotification(context: Context): Notification {
    val notificationBuilder = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        NotificationCompat.Builder(context, SCREENSAVER_SERVICE_CHANNEL_ID)
    } else {
        @Suppress("DEPRECATION")
        NotificationCompat.Builder(context)
    }
    ...
    return notificationBuilder.setOngoing(true)
        .setSmallIcon(R.mipmap.ic_launcher)
        .setContentTitle(context.getString(R.string.screensaver_notification_title))
        .setPriority(NotificationCompat.PRIORITY_LOW)
        .setCategory(Notification.CATEGORY_SERVICE)
        .setContentIntent(pendingIntent)
        .build()
